...
class Ui_Form_3(object):
    def Cart2GeoInDirect(self):
        ...
        Lambda = np.arctan(Y / X)

        phi_0 = np.arctan(Z / np.sqrt(X ** 2 + Y ** 2) * (1 + (e_2 / (1 - e_2))))
        N_0 = a / np.sqrt(1 - (e_2 * np.sin(phi_0) ** 2))

        phi_i = np.arctan(Z / np.sqrt(X ** 2 + Y ** 2) * (1 + (e_2 * N_0 * np.sin(phi_0) / (Z))))
        N_i = a / np.sqrt(1 - (e_2 * np.sin(phi_0) ** 2))

        erreur = 0.00000001

        while (abs(phi_i - phi_0) < erreur):
            phi_0 = phi_i
            N_0 = N_i
            phi_i = np.arctan(Z / np.sqrt(X ** 2 + Y ** 2) * (1 + (e_2 * N_0 * np.sin(phi_0) / (Z))))
        ...
        elevation = (math.sqrt(X ** 2 + Y ** 2)) * np.cos(phi) + (Z * np.sin(phi)) - a * math.sqrt(1 - (e_2 * np.sin(phi) ** 2))

        (phi_d, phi_m, phi_s) = Conversion.rad2dms2(phi)
        (lambda_d, lambda_m, lambda_s) = Conversion.rad2dms2(Lambda)
        self.lineEdit.setText(str(phi_d))
        self.lineEdit_2.setText(str(phi_m))
        self.lineEdit_3.setText(str(phi_s))

        self.lineEdit_4.setText(str(lambda_d))
        self.lineEdit_5.setText(str(lambda_m))
        self.lineEdit_6.setText(str(lambda_s))

        self.lineEdit_7.setText(str(elevation))
...
"")
        ...
        self.pushButton.clicked.connect(self.Cart2GeoInDirect)
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "CartoGeo(InDirect)"))
        Form.setWindowIcon(QtGui.QIcon("icon.png"))
        self.label_17.setText(_translate("Form", "<html><head/><body><p align=\"center\"><span style=\" color:#7b7561;\">Transformation des coordonnées cartésiennes </span></p><p align=\"center\"><span style=\" color:#7b7561;\">aux coordonnées géographiques (InDirect)</span></p></body></html>"))
